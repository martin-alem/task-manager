...
(function () {
	const menu = document.querySelector("main header .menu .fas");
	const close = document.querySelector("main .weather_and_task i.fa-times");
	const sideMenu = document.querySelector("main .weather_and_task");
	...
	close.addEventListener("click", function () {
		sideMenu.classList.replace("sideMenu", "closeSide");
		setTimeout(function () {
			sideMenu.style.opacity = "0";
			sideMenu.style.display = "none";
			sideMenu.classList.remove("closeSide");
		}, 500);
	});

	window.addEventListener("resize", function () {
		if (window.innerWidth >= 1196) {
			sideMenu.classList.remove("closeSide", "sideMenu");
